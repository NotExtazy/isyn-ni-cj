...
function LoadEquipmentList() {
    ...
    $.post(ROUTE_URL, { action: 'LoadEquipmentList', AssetType: assetType, Status: status }, function (res) {
        ...
        var thead = '';
        if (tbl === 'furniture') {
            thead = '<tr><th></th><th>Description</th><th>Property No</th><th>Date Acquired</th><th>Ref No</th><th>Unit Cost/Transferred</th><th>No of Units</th><th>Total Cost</th><th>Est Useful Life</th><th>No of Months</th><th>Month Started Depr\'n</th><th>Accum Depr\'n (Prev Year)</th><th>Depr\'n This Year</th><th>Accum Depr\'n (As Of Date)</th><th>Net Book Value</th><th>Status</th></tr>';
        } else if (tbl === 'transpo') {
        ...
        $.each(res.LIST, function (i, v) {
            ...
            if (tbl === 'furniture') {
                row = '<tr data-id="' + v.ID + '">' +
                    ...
                    '<td>' + numberFmt(v.TotalCostTransferred) + '</td><td>' + (v.NoOfUnits||'-') + '</td>' +
                    '<td>' + numberFmt(v.TotalCost) + '</td>' +
                    '<td>' + (v.EstUsefulLife||'-') + '</td><td>' + (v.NoOfMonths||'-') + '</td>' +
                    '<td>' + (v.MonthStartedDepr||'-') + '</td><td>' + numberFmt(v.AccumDeprPrevYear) + '</td>' +
                    '<td>' + numberFmt(v.DeprThisYear) + '</td><td>' + numberFmt(v.AccumDeprAsOfDate) + '</td>' +
        ...
        if ($.fn.DataTable.isDataTable('#equipmentTbl')) {
            $('#equipmentTbl').DataTable().clear().destroy();
        }
        
        // Clear pagination containers before initializing
        $('#equipmentTbl_info_container').empty();
        $('#equipmentTbl_paginate_container').empty();

        equipmentTable = $('#equipmentTbl').DataTable({
            paging: true, pageLength: 10, lengthChange: false, info: true, dom: 'tip',
            drawCallback: function() {
                // Move pagination controls to footer after each draw
                var info = $('.dataTables_info').detach();
                var paginate = $('.dataTables_paginate').detach();
                $('#equipmentTbl_info_container').empty().append(info);
                $('#equipmentTbl_paginate_container').empty().append(paginate);
            }
        });
...
function SavePPE(type) {
    ...
    if (type === 'furniture') {
        ...
        data.DeprThisYearSummary = $('#ff_deprthisyear').val(); // Same as DeprThisYear
        ...
        data.DeprThisYearSummary = $('#tr_deprthisyear').val(); // Same as DeprThisYear
        ...
        data.DeprThisYearSummary = $('#lh_deprthisyear').val(); // Same as DeprThisYear
...
function openEditModal() {
    ...
    // Determine asset type and modal
    var assetType = data.AssetType || $('#filterAssets').val();
    var modalMap = {
        'Fur Fix Equip':     'modalFurniture',
        'Transpo Equipment': 'modalTranspo',
        'Leasehold Imp':     'modalLeasehold'
    ...
    var modalId = modalMap[assetType];
    if (!modalId) {
        Swal.fire({ icon: 'error', title: 'Unknown asset type', text: 'Cannot determine which form to open.' });
        return;
    }
    ...
    if (assetType === 'Fur Fix Equip') {
        $('#ff_id').val(data.ID || '');
        $('#ff_transactionid').val(data.TransactionID || data.PropertyNo || '');
        $('#ff_description').val(data.Description || '');
        $('#ff_dateacquired').val(data.DateAcquired || '');
        $('#ff_refno').val(data.RefNo || '');
        $('#ff_noofunits').val(data.NoOfUnits || 1);
        $('#ff_totalcosttransferred').val(data.TotalCostTransferred || 0);
        $('#ff_totalcost').val(data.TotalCost || 0);
        $('#ff_disposaltransferout').val(data.DisposalTransferOut || 0);
        $('#ff_disposalreclass').val(data.DisposalReclass || 0);
        $('#ff_estusefullife').val(data.EstUsefulLife || 5);
        $('#ff_noofmonths').val(data.NoOfMonths || 0);
        $('#ff_monthstarteddepr').val(data.MonthStartedDepr || '');
        $('#ff_monthlydepr').val(data.MonthlyDepr || 0);
        $('#ff_accumdeprprevyear').val(data.AccumDeprPrevYear || 0);
        $('#ff_deprthisyear').val(data.DeprThisYear || 0);
        $('#ff_accumdeprasofdate').val(data.AccumDeprAsOfDate || 0);
        $('#ff_netbookvalue').val(data.NetBookValue || 0);
        $('#ff_lapjan').val(data.LapJan || 0);
        $('#ff_lapfeb').val(data.LapFeb || 0);
        $('#ff_lapmar').val(data.LapMar || 0);
        $('#ff_lapapr').val(data.LapApr || 0);
        $('#ff_lapmay').val(data.LapMay || 0);
        $('#ff_lapjun').val(data.LapJun || 0);
        $('#ff_lapjul').val(data.LapJul || 0);
        $('#ff_lapaug').val(data.LapAug || 0);
        $('#ff_lapsep').val(data.LapSep || 0);
        $('#ff_lapoct').val(data.LapOct || 0);
        $('#ff_lapnov').val(data.LapNov || 0);
        $('#ff_lapdec').val(data.LapDec || 0);
        $('#ff_laptotal').val(data.LapTotal || 0);
    } else if (assetType === 'Transpo Equipment') {
        $('#tr_id').val(data.ID || '');
        $('#tr_transactionid').val(data.TransactionID || data.PropertyNo || '');
        $('#tr_description').val(data.Description || '');
        $('#tr_dateacquired').val(data.DateAcquired || '');
        $('#tr_refno').val(data.RefNo || '');
        $('#tr_acquisitioncost').val(data.AcquisitionCost || 0);
        $('#tr_noofunits').val(data.NoOfUnits || 1);
        $('#tr_totalcost').val(data.TotalCost || 0);
        $('#tr_estusefullife').val(data.EstUsefulLife || 5);
        $('#tr_noofmonths').val(data.NoOfMonths || 0);
        $('#tr_monthstarteddepr').val(data.MonthStartedDepr || '');
        $('#tr_monthlydepr').val(data.MonthlyDepr || 0);
        $('#tr_accumdeprprevyear').val(data.AccumDeprPrevYear || 0);
        $('#tr_deprthisyear').val(data.DeprThisYear || 0);
        $('#tr_accumdeprasofdate').val(data.AccumDeprAsOfDate || 0);
        $('#tr_netbookvalue').val(data.NetBookValue || 0);
        $('#tr_lapjan').val(data.LapJan || 0);
        $('#tr_lapfeb').val(data.LapFeb || 0);
        $('#tr_lapmar').val(data.LapMar || 0);
        $('#tr_lapapr').val(data.LapApr || 0);
        $('#tr_lapmay').val(data.LapMay || 0);
        $('#tr_lapjun').val(data.LapJun || 0);
        $('#tr_lapjul').val(data.LapJul || 0);
        $('#tr_lapaug').val(data.LapAug || 0);
        $('#tr_lapsep').val(data.LapSep || 0);
        $('#tr_lapoct').val(data.LapOct || 0);
        $('#tr_lapnov').val(data.LapNov || 0);
        $('#tr_lapdec').val(data.LapDec || 0);
        $('#tr_laptotal').val(data.LapTotal || 0);
    } else if (assetType === 'Leasehold Imp') {
        $('#lh_id').val(data.ID || '');
        $('#lh_transactionid').val(data.TransactionID || data.PropertyNo || '');
        $('#lh_description').val(data.Description || '');
        $('#lh_dateacquired').val(data.DateAcquired || '');
        $('#lh_refno').val(data.RefNo || '');
        $('#lh_acquisitioncost').val(data.AcquisitionCost || 0);
        $('#lh_noofunits').val(data.NoOfUnits || 1);
        $('#lh_totalcost').val(data.TotalCost || 0);
        $('#lh_estusefullife').val(data.EstUsefulLife || 5);
        $('#lh_noofmonths').val(data.NoOfMonths || 0);
        $('#lh_monthstarteddepr').val(data.MonthStartedDepr || '');
        $('#lh_monthlydepr').val(data.MonthlyDepr || 0);
        $('#lh_accumdeprprevyear').val(data.AccumDeprPrevYear || 0);
        $('#lh_deprthisyear').val(data.DeprThisYear || 0);
        $('#lh_accumdeprasofdate').val(data.AccumDeprAsOfDate || 0);
        $('#lh_netbookvalue').val(data.NetBookValue || 0);
        $('#lh_lapjan').val(data.LapJan || 0);
        $('#lh_lapfeb').val(data.LapFeb || 0);
        $('#lh_lapmar').val(data.LapMar || 0);
        $('#lh_lapapr').val(data.LapApr || 0);
        $('#lh_lapmay').val(data.LapMay || 0);
        $('#lh_lapjun').val(data.LapJun || 0);
        $('#lh_lapjul').val(data.LapJul || 0);
        $('#lh_lapaug').val(data.LapAug || 0);
        $('#lh_lapsep').val(data.LapSep || 0);
        $('#lh_lapoct').val(data.LapOct || 0);
        $('#lh_lapnov').val(data.LapNov || 0);
        $('#lh_lapdec').val(data.LapDec || 0);
        $('#lh_laptotal').val(data.LapTotal || 0);
    }
    ...
    if (targetEl) {
        ...
        Swal.fire({ icon: 'error', title: 'Modal not found', text: 'Cannot open edit form.' });
    }
}
        ...
        try {
            ...
            allTypesArray.forEach(function(assetType) {
                ...
                typeCols.forEach(function(c) {
                    if (lapKeys.indexOf(c.key) !== -1 || c.key === 'LapTotal') return;
                    if (c.key === 'Status') return; // Skip Status column in PDF
                    if (c.spacer) {
        ...
        try {
            ...
            function buildSheetWithCols(activeRows, disposedRows, sheetTitle, sheetCols) {
                ...
                sheetCols.forEach(function(c, i) {
                    ...
                    } else if (c.key === 'TotalCostTransferred') {
                        worksheet.getColumn(i + 1).width = 10; // Reduced width
                    } else if (c.key === 'NoOfUnits') {
                        worksheet.getColumn(i + 1).width = 8; // Reduced width
                    } else if (c.key === 'EstUsefulLife') {
                        worksheet.getColumn(i + 1).width = 10; // Reduced width
                    } else if (c.key === 'NoOfMonths') {
                        worksheet.getColumn(i + 1).width = 8; // Reduced width
                    } else if (c.key === 'MonthStartedDepr') {
                        worksheet.getColumn(i + 1).width = 12; // Reduced width
                    } else if (c.key === 'DeprThisYear') {
                        worksheet.getColumn(i + 1).width = 10; // Reduced width
                    } else if (c.key === 'AccumDeprAsOfDate') {
                        worksheet.getColumn(i + 1).width = 12; // Reduced width
                    } else if (c.key === 'DeprThisYearSummary') {
                        worksheet.getColumn(i + 1).width = 12; // Reduced width
                    } else if (c.key === 'AccumAsOfDate') {
                        worksheet.getColumn(i + 1).width = 12; // Same as DeprThisYearSummary
...
function _getExportCols(tbl) {
    if (tbl === 'furniture') {
        return [
            { key:'Description',        label:'TYPE OF ASSET/\nDESCRIPTION',      pdfLabel:'TYPE OF ASSET/DESCRIPTION',           num:false },
            { key:'PropertyNo',         label:'PROPERTY NO.',                   pdfLabel:'PROPERTY NO.',                        num:false },
            { key:'DateAcquired',       label:'DATE\nACQUIRED',                  pdfLabel:'DATE ACQUIRED',                       num:false },
            { key:'RefNo',              label:'REF NO.',                        pdfLabel:'REF NO.',                             num:false },
            { key:'TotalCostTransferred',label:'UNIT COST/\nTRANSFERRED',         pdfLabel:'UNIT COST/<br>TRANSFERRED',           num:true  },
            { key:'NoOfUnits',          label:'NO. OF\nUNITS',                   pdfLabel:'NO. OF<br>UNITS',                     num:true  },
            { key:'TotalCost',          label:'TOTAL COST',                     pdfLabel:'TOTAL COST',                          num:true  },
            { key:'EstUsefulLife',      label:'EST. USEFUL\nLIFE',               pdfLabel:'EST. USEFUL<br>LIFE',                 num:true  },
            { key:'NoOfMonths',         label:'NO. OF\nMONTHS',                  pdfLabel:'NO. OF<br>MONTHS',                    num:true  },
            { key:'MonthStartedDepr',   label:'MONTH STARTED\nDEPR\'N.',         pdfLabel:'MONTH STARTED<br>DEPR\'N.',           num:false },
            { key:'AccumDeprPrevYear',  label:'ACCUM. DEPR\'N.\nPREV YEAR',      pdfLabel:'ACCUM.<br>DEPR\'N.<br>PREV YEAR',     num:true  },
            { key:'DeprThisYear',       label:'DEPR\'N.\nTHIS YEAR',             pdfLabel:'DEPR\'N.<br>THIS YEAR',               num:true  },
            { key:'AccumDeprAsOfDate',  label:'ACCUM. DEPR\'N.\n(AS OF DATE)',   pdfLabel:'ACCUM.<br>DEPR\'N.<br>(AS OF DATE)',  num:true  },
            { key:'NetBookValue',       label:'NET BOOK\nVALUE',                 pdfLabel:'NET BOOK<br>VALUE',                   num:true  },
            { key:'_spacer1',           label:'',                               pdfLabel:'',                                    num:false, spacer:true },
            { key:'DeprThisYearSummary',label:'DEPR\'N. THIS YEAR',             pdfLabel:'DEPR\'N.<br>THIS YEAR',               num:true  },
            { key:'AccumAsOfDate',      label:'ACCUM.\nAS OF DATE',              pdfLabel:'ACCUM.<br>AS OF DATE',                num:true  },
            ...
            { key:'Status',             label:'STATUS',                         pdfLabel:'STATUS',                              num:false }
        ];
    } else if (tbl === 'transpo') {
        return [
            { key:'Description',        label:'TYPE OF ASSET/\nDESCRIPTION',      pdfLabel:'TYPE OF ASSET/DESCRIPTION',           num:false },
            ...
            { key:'DateAcquired',       label:'DATE\nACQUIRED',                  pdfLabel:'DATE ACQUIRED',                       num:false },
            { key:'RefNo',              label:'REF NO.',                        pdfLabel:'REF NO.',                             num:false },
            { key:'AcquisitionCost',    label:'ACQUISITION\nCOST',               pdfLabel:'ACQUISITION COST',                    num:true  },
            { key:'NoOfUnits',          label:'NO. OF\nUNITS',                   pdfLabel:'NO. OF<br>UNITS',                     num:true  },
            { key:'TotalCost',          label:'TOTAL COST',                     pdfLabel:'TOTAL COST',                          num:true  },
            { key:'EstUsefulLife',      label:'EST. USEFUL\nLIFE',               pdfLabel:'EST. USEFUL<br>LIFE',                 num:true  },
            { key:'NoOfMonths',         label:'NO. OF\nMONTHS',                  pdfLabel:'NO. OF<br>MONTHS',                    num:true  },
